...
# Streamlit Page Config
st.set_page_config(page_title='Yahoo Finance Analysis', page_icon='📈', layout='wide', initial_sidebar_state='auto')
...
if ticker and start_date and end_date:
    ...
    if df is not None:
        ...
        try:
            ...
            trend_fig = px.line(df, x='Date', y='Trend', title='Trend Component')
            seasonal_fig = px.line(df, x='Date', y='Seasonal', title='Seasonal Component')
            residual_fig = px.line(df, x='Date', y='Residual', title='Residual Component')
        ...
        # Forecasting with SARIMA
        if view_option in ['Forecast', 'Both']:
            st.subheader(f'🔮 {ticker} Stock Price Forecasting')
            ...
            try:
                ...
                forecast_fig.add_trace(go.Scatter(x=test.index, y=test, mode='lines', name='Actual'))
                forecast_fig.add_trace(go.Scatter(x=test.index, y=forecast, mode='lines', name='Forecast', line=dict(color='red')))
                st.plotly_chart(forecast_fig)

                # Future Forecast for 1 Year
                forecast_steps = 365
                future_forecast = results.predict(start=len(df), end=len(df) + forecast_steps - 1, dynamic=True)
                future_dates = pd.date_range(start=df.index[-1], periods=forecast_steps + 1, freq='B')[1:]
                forecast_df = pd.DataFrame({'Date': future_dates, 'Forecast': future_forecast})
